...
class MoveItToEgm(Node):
    ...
    def _on_plan(self, msg: DisplayTrajectory) -> None:
        ...
        # Always dump latest plan to a watch-file so an external "drive" CLI
        # can pick it up. Wrote to /tmp/last_plan.yaml in canonical format.
        try:
            import yaml
            Path('/tmp/last_plan.yaml').write_text(yaml.safe_dump({
                'waypoints': [{
                    'name': 'last_moveit_plan',
                    'joints': [round(v, 6) for v in target_rad],
                    'planned_utc': datetime.now(timezone.utc).isoformat(timespec='seconds').replace('+00:00', 'Z'),
                }],
            }, sort_keys=False))
        except Exception as e:
            self.get_logger().warn(f'could not dump plan: {e}')

        # Confirm mode (default). Set AUTO_DRIVE=1 to skip the prompt.
        if os.environ.get('AUTO_DRIVE') == '1':
            if self._driver_thread and self._driver_thread.is_alive():
                self.get_logger().warn('drive in progress, ignoring new plan')
                return
            self.get_logger().info('AUTO_DRIVE: driving in 3 sec (Ctrl-C to cancel)')
            self._driver_thread = threading.Thread(
                target=self._delayed_drive, args=(target_rad,), daemon=True)
            self._driver_thread.start()
        else:
            self.get_logger().info('Type "drive" to execute, "save <name>" to store, "list" to recheck.')

    def _delayed_drive(self, target_rad: list[float]) -> None:
        for i in range(3, 0, -1):
            self.get_logger().info(f'  driving in {i}...')
            time.sleep(1.0)
        self.get_logger().info('  >>> EGM drive START <<<')
        ok = egm_drive(target_rad)
        self.get_logger().info(f'  >>> drive {"OK" if ok else "FAILED"} <<<')

    def drive_last(self) -> bool:
        t = self.get_last_target_rad()
        if t is None:
            return False
        if self._driver_thread and self._driver_thread.is_alive():
            return False
        self._driver_thread = threading.Thread(
            target=self._delayed_drive, args=(t,), daemon=True)
        self._driver_thread.start()
        return True
